...
```
make CUDA_ARCH=?
./svd_integrated_imager_gpu Measurement_Set.ms Image_Size Cell_Size Snapshot_Index Output_Name.fits
```
...
```
Here, **Visreal_input.fits**, **Visimag_input.fits**, **B_input.fits**, and **V_input.fits** are the input files (in FITS format) corresponding to the real components of visibilities, the imaginary components of visibilities, the (centred) SVDed baseline matrix, and the V matrix in the SVD, respectively. The remaining arguments are as their names suggest, where **Image_Size** is an integer (e.g., if you input 128, it means the image size is $128 \times 128$ pixels), **Number_of_Rows = Number_of_Baselines * Number_of_Frequency_Channels** is an integer, **Cell_Size** is in units of radians, and the last argument is the name of the output file which should end with '.fits'.
